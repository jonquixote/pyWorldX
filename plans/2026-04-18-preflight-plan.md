...
**File:** `data_pipeline/map.py`
**Problem:** `world3_reference_food_per_capita` (kg/person/yr) and `faostat_food_balance_historical` (kcal/capita/day) are merged under the same pipeline entity `food_per_capita` with no conversion applied. The two series differ by a factor of ~1,000 at 1970, which blows out the food-sector NRMSD.

**Fix:**
- Add a `unit_conversion` field to the entity definition schema (or a `ConversionStep` dataclass).
- Apply the conversion `kcal/capita/day → kg/person/yr` using the factor `1 kcal/day = 0.0547 kg/yr` (at ~1800 kcal/kg cereal equivalent) before merging.
- Alternatively, keep the two series as separate entities and let `DataBridge._normalize_to_index()` handle alignment — this is the safer short-term approach.
...
**Acceptance test:** `get_initial_conditions()` (no arguments) returns the 1970 stock vector; `get_initial_conditions(target_year=1900)` still works when explicitly requested.
...
**Problem:** `service_capital` (SC), `industrial_capital` (IC), and `arable_land` (AL) each receive data from 2–3 connectors with incompatible units and no arbitration logic. Last-write in Python dict iteration order determines which series is used — this is non-deterministic across Python versions.

**Fix:**
- Add a `source_priority` list to each multi-source entity in `ENTITY_TO_ENGINE_MAP`, e.g.:
  ```python
  "service_capital": {
      "engine_var": "SC",
      "sources": ["world_bank_capital_stock", "penn_world_table"],
      "source_priority": ["penn_world_table", "world_bank_capital_stock"],
      "unit": "trillion_usd_2017ppp"
  }
  ```
...
**Problem:** There is no continuous observed nonrenewable resource stock series in the pipeline. `usgs_mcs` delivers metadata strings. The World3 reference trajectory is the only current NR input — circular calibration. This makes the resource sector entirely synthetic.
...
- Map to entity `nonrenewable_resources_proved_reserves` with `unit="EJ"`.
- Add conversion: `EJ → World3 NR resource units` (calibrate scalar against 1970 World3-03 NR initial value of ~1.0 × 10¹²).
- Tag as `layer=1` (observed proxy) vs. `world3_reference_nonrenewable_resources` as `layer=0` (structural reference).
...
def _normalize_to_index(self, series: pd.Series, base_year: int) -> pd.Series:
    ...
    if base_val == 0 or pd.isna(base_val):
        ...
        if nonzero.empty:
            raise DataBridgeError(f"Cannot normalize: no non-zero base value near {base_year}")
        base_val = nonzero.iloc[0]
        logger.warning("Zero base value at %d; using %d instead", base_year, nonzero.index[0])
...
```
Expected output columns: `entity | source | years_covered | gap_count | base_year_value | base_year_nonzero`. Any entity with `base_year_nonzero=False` is a T1/T2 blocker that must be resolved.

### Gate 4 — Full Test Suite
```bash
pytest tests/ -x -q --tb=short
```
Expected: all tests pass. Zero `XFAIL` items that were passing before. Pay particular attention to `test_databridge.py` and `test_empirical_calibration.py` (to be written per the DataBridge integration plan).

### Gate 5 — End-to-End Dry Run
```bash
python -m pyworldx.calibration.empirical --dry-run --train-window 1970-2010 --holdout-window 2010-2023
```
Expected: run completes without `UnitMismatchError`, `DataBridgeError`, or `NaN` in reported NRMSD. The `--dry-run` flag should skip the Nelder-Mead optimizer and report data coverage only.
...
|---|---|---|---|
| 1 | Scenario for empirical calibration | `Standard Run` only vs. accept `scenario` arg | **Recommended: accept arg, default `standard_run`** — see DataBridge integration plan |
| 2 | NR proxy layer weighting | BP reserves as sole Layer 1 vs. blend with WEO data | Unresolved — defer to post-preflight |
| 3 | World3ReferenceConnector scope | Layer 0 (structural only, excluded from objective) vs. Layer 1 (included with low weight) | **Recommended: Layer 0 only** — see preflight audit rationale |
...
- [ ] All 5 Tier 2 issues resolved and committed
